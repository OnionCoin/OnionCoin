...
uint256 hashGenesisBlock("0x0059bde0c16ca63a0a42e0336021f05e1b290393b8af3e1cb2cbf18e80f92dc5");
...
const string strMessageMagic = "OnionCoin Signed Message:\n";
...
int64 static GetBlockValue(int nHeight, int64 nFees,uint256 prevHash)
{
	...
	if(nHeight == 1)
	{
		nSubsidy = 500000 * COIN; //small premine  => 0.5% coins
	}
    else if(nHeight < 250)
    {
       nSubsidy = 1 * COIN; //no instamine
    }
    else
    {
	
		if(rand < 5000) //50% - 5000/10000
			nSubsidy = 20 * COIN;
		else if(rand < 7500) //25% - 2500/10000
			nSubsidy = 40 * COIN;
		else if(rand < 8750) //12,5% - 1250/10000
			nSubsidy = 80 * COIN;
		else if(rand < 9750) //10% - 1000/10000
			nSubsidy = 100 * COIN;
		else //2,5%	- 250/10000
			nSubsidy = 400 * COIN;
		//1440 block x 50coins(average) = 72k new coins per day
    
    }
    
	nSubsidy >>= (nHeight / 1051200); //2 years
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // OnionCoin: 1 days
static const int64 nTargetSpacing = 60; // OnionCoin: 1 minute blocks
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal OnionCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "OnionCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Peel this onion 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391433193; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 7955;

        if (fTestNet)
        {
            block.nTime    = 1391433193;
            block.nNonce   = 7955;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x2ed9327778c4ec5a40e3cff70b0c56f03c1b7f70d4cef130dc47ef29ec1cb73b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xd9, 0xb7, 0x55 };
